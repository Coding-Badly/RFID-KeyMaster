...
def test_ping_pong_by_driver_name(caplog):
    ...
    tm1 = DriverGroup()
    tm2 = tm1.add(Ping1(config=None))
    tm3 = tm1.add(Pong1(config=None))
    assert tm1['Ping1'] == tm2
...
def test_ping_pong_level_two_tree(caplog):
    ...
    lft_ping = lft.add(Ping1(config=None))
    lft_pong = lft.add(Pong1(config=None))
    rgt = root.add(DriverGroup('right'))
    rgt_pong = rgt.add(Pong1(config=None))
...
def test_ping_pong_level_three_tree(caplog):
    ...
    lft_lft_ping = lft_lft.add(Ping1(config=None))
    lft_lft_pong = lft_lft.add(Pong1(config=None))
    lft_rgt_ping = lft_rgt.add(Ping1(config=None))
    lft_rgt_pong = lft_rgt.add(Pong1(config=None))
    rgt_lft_ping = rgt_lft.add(Ping1(config=None))
    rgt_lft_pong = rgt_lft.add(Pong1(config=None))
    #
    rgt_rgt_pong = rgt_rgt.add(Pong1(config=None))
...
def test_ping_pong_run_1(caplog):
    caplog.set_level(logging.INFO)
    root = DriverGroup('root')
    ping = root.add(Ping1(config=None))
    pong = root.add(Pong1(config=None))
    root.setup()
    root.start()
    # rmv ping.join()
    # rmv pong.join()
...
def x_test_RunForSeconds_simple_test(caplog):
    root = DriverGroup('root')
    tm1 = root.add(RunForSeconds(config={'seconds':1.0}))
    root.setup()
    root.start()
    tm1.join()
    root.teardown()
...
def test_one_second_of_ping_pong_1(caplog):
    ...
    root = DriverGroup('root')
    tm1 = root.add(RunForSeconds(config={'seconds':1.0}))
    tm2 = root.add(Ping1(config={'stop_at_1000':False}))
    tm3 = root.add(Pong1(config=None))
    root.setup()
    root.start()
    # rmv tm1.join()
    # rmv tm2.join()
...
def test_one_second_of_ping_pong_2(caplog):
    caplog.set_level(logging.INFO)
    root = DriverGroup('root')
    rfs = root.add(RunForSeconds(config={'seconds':1.0}))
    ping = root.add(Ping2(config=None))
    pong = root.add(Pong1(config=None))
    root.setup()
    root.start()
    # rmv rfs.join()
...
def test_flatten_3(caplog):
    ...
    rgt_lft_ping = rgt_lft.add(Ping1(config=None))
    rgt_lft_pong = rgt_lft.add(Pong1(config=None))
    #rgt_rgt_ping = rgt_rgt.add(Ping1(config=None))
    rgt_rgt_pong = rgt_rgt.add(Pong1(config=None))
