...
class Permission():
    """Permission is a single named permission similar to a Django permission.
    """
    # pylint: disable=protected-access
    def __init__(self, codename, name=None, context=None):
...
class Group():
    """Group is a named group similar to a Django group to which any number of members can belong.
    """
...
def repr_init_list(dict_to_use, indent):
    """Build an initialization list for a list of Permissions or a list of Groups.
    """
    # pylint: disable=protected-access
    rv = '['
    lft = ''
...
class SecurityContext():
    """SecurityContext is a list of Groups and Permissions that forms a security context.
    """
    # pylint: disable=no-else-return
    # pylint: disable=protected-access
    ...
    def __eq__(self, other):
        if isinstance(other, SecurityContext) \
            ...
            for key, gr1 in self._groups.items():
                gr2 = other._groups[key]
                if gr1._permissions != gr2._permissions:
                    return False
            return True
        else:
            return False
    def __repr__(self):
        # rmv p1 = [i1._get_init_tuple() for i1 in self._permissions.values()]
    ...
    def _iterate_add_permissions(self, rv, permissions):
        for rover in permissions:
            if isinstance(rover, (list, tuple)) and (len(rover) > 1):
    ...
    def add_permission(self, codename, name):
        """Add a single named Permission to the SecurityContext.
        """
        rv = self._permissions.get(codename, None)
        if not rv:
            rv = Permission(codename, name, self)
            # rmv rv._context = self
    ...
    def add_group(self, name, permissions=None):
        """Add a single named Group to the SecurityContext.
        """
        self._effective_rights.clear()
        return self._add_group(name, permissions)
    def add_groups(self, groups):
        """Add a list of named Groups to the SecurityContext.
        """
        self._effective_rights.clear()
    ...
    def get_effective_rights(self, groups):
        """Return a set of Permissions included in the list of groups.
